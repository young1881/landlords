...
public class Common {
    ...
    public static void rePosition(GameFrame m, ArrayList<Poker> list, int flag) {
        Point p = new Point();
        if (flag == 0) {
            p.x = 50;
    ...
    public static void getTwoTwo(ArrayList<Poker> list, Model model) {
        ...
        for (int i = 0, len = l.size(); i < len; i++) {
            ...
            if (k - i >= 2) {
                ...
                for (int j = i; j < k; j++) {
                    ss.append(l.get(j)).append(",");
                    del.add(l.get(j));
                }
                ss.append(l.get(k));
                model.a112233.add(ss.toString());
                del.add(l.get(k));
                i = k;
            }
        }
    ...
    public static int checkCards(ArrayList<Poker> c, ArrayList<ArrayList<Poker>> current, GameFrame m) {
        ArrayList<Poker> currentlist;
        if (m.time[0].getText().equals("Pass"))
